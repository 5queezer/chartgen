...
impl Default for Rsi {
    fn default() -> Self { Self { length: 14, overbought: 70.0, oversold: 30.0 } }
}
...
impl Indicator for Rsi {
    ...
    fn compute(&self, data: &OhlcvData) -> PanelResult {
        ...
        PanelResult {
            lines: vec![
                Line { y: rsi, color: rgba(0xc33ee1, 0.8), width: 2, label: Some("RSI".into()) },
            ],
            hlines: vec![
                HLine { y: self.overbought, color: rgba(0xe13e3e, 0.3) },
                HLine { y: self.oversold, color: rgba(0x3ee145, 0.3) },
                HLine { y: 50.0, color: rgba(0x787b86, 0.2) },
            ],
